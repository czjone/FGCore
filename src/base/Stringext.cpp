...
using namespace std;
std::string StrTool::tolower(const std::string &val){
...
std::string StrTool::touper(const std::string &val){
	std::string str(val.c_str());
	for (int i = 0; i < str.length(); i++){
		char c = str[i];
		if (c >= 'a' && c <= 'z') str[i] -= ('A' - 'a');
...
std::vector<std::string> StrTool::split(std::string& str, std::string& splitstr){
...
string& StrTool::replace_first(string&str, const string&  old_value, const string&  new_value)
{
...
string& StrTool::replace_all(string&str, const string&  old_value, const string&  new_value)
{
	while (true)   {
		string::size_type   pos(0);
		if ((pos = str.find(old_value)) != string::npos)
...
string& StrTool::replace_all_distinct(string&   str, const   string&   old_value, const   string&   new_value)
{
